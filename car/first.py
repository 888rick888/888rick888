#-- rf_study.py---->first.py   @pzs                          --
#-- change neural network from Xception to EfficientNetB7    --
#-- annotate eht part of tensorboard and thread              --
...
class CarEnv:
    ...
    def reset(self):
        ...
        self.transform = random.choice(self.world.get_map().get_spawn_points()[20:40])
...
class DQNAgent:
    ...
    def train(self):
        ...
        with self.graph.as_default():
            ...
            self.model.fit(np.array(x)/255,np.array(y),batch_size=TRAINING_BATCH_SIZE,verbose=0,shuffle=False,callbacks=[self.tensorboard] if False else None)  #remove callbacks @pzs (if log_this_step)
        if log_this_step:
